...
import com.mybitcoin.wallet.ui.InputParser.BinaryInputParser;
import com.mybitcoin.wallet.ui.InputParser.StringInputParser;
import com.mybitcoin.wallet.util.CrashReporter;
import com.mybitcoin.wallet.util.Crypto;
import com.mybitcoin.wallet.util.HttpGetThread;
import com.mybitcoin.wallet.util.Iso8601Format;
import com.mybitcoin.wallet.util.Nfc;
...
//        final View view = getLayoutInflater().inflate(R.layout.customtitle, null);
        ...
        //accessNetworkState();
       Button btn1 = (Button)findViewById(R.id.to_send_activity);
        btn1.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View view){
                //startActivity(new Intent(WalletActivity.this, SendCoinsActivity.class));
                startActivity(new Intent(WalletActivity.this,SelectOperationActivity.class));
            }
        });
       Button btn2 = (Button)findViewById(R.id.to_receive_activity);
        btn2.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View view){
                startActivity(new Intent(WalletActivity.this,ScanQRResultActivity.class));
            }
        });
        Button btn3 = (Button)findViewById(R.id.to_exchangeRate_activity);
	...
	public boolean onOptionsItemSelected(final MenuItem item)
	{
		switch (item.getItemId())
		{
			...
			case R.id.wallet_options_help:
				...
				return true;*/
            case R.id.wallet_send_message:
                return true;
            case R.id.wallet_print_message:
//                startActivity(new Intent(this, QRCodeActivity.class));
                return true;
            case R.id.wallet_setup_serialport:
...
//			log.info("exported " + keys.size() + " private keys to " + file);
            log.info("exported " + cipherText + " private keys to " + file);
		}
		catch (final IOException x)
		{
			final DialogBuilder dialog = DialogBuilder.warn(this, R.string.import_export_keys_dialog_failure_title);
    ...
    public void    accessNetworkState(){
        Context context = getApplicationContext();
        ConnectivityManager conn = (ConnectivityManager)context.getSystemService(Context.CONNECTIVITY_SERVICE);
        NetworkInfo[] info = conn.getAllNetworkInfo();
        if(info != null){
            for(int i=0;i<info.length;i++){
                if(info[i].getType() == ConnectivityManager.TYPE_ETHERNET )
                    Toast.makeText(context,"连接上有线网",Toast.LENGTH_SHORT).show();
                else
                    Toast.makeText(context,"未连接上有线网",Toast.LENGTH_SHORT).show();
            }
        }
        /*IntentFilter filter = new IntentFilter();
        filter.addAction(ConnectivityManager.CONNECTIVITY_ACTION);
        filter.addAction(ConnectivityManager.EXTRA_NO_CONNECTIVITY);
        filter.addAction(ConnectivityManager.EXTRA_IS_FAILOVER);
    ...
    /*public BroadcastReceiver  mEthernetReceiver = new BroadcastReceiver(){
        @Override
        public void onReceive(Context context,Intent intent){
            if(intent.getAction().equals(ConnectivityManager.ACTION_ETHERNET_STATE_CHANGED)){
                ...
                if(state.equals(ConnectivityManager.EXTRA_ETH_CONNECT)){
                    Toast.makeText(context,"连接上有线网",Toast.LENGTH_SHORT);
                }else if(state.equals(ConnectivityManager.EXTRA_ETH_UNLINK)){
                    Toast.makeText(context,"已断开有线网",Toast.LENGTH_SHORT);
                }
            }
        }
    };*/
