...
import com.mybitcoin.wallet.AddressBookProvider;
...
import com.mybitcoin.wallet.WalletBalanceWidgetProvider;
import com.mybitcoin.wallet.ui.CashTradeShowCashQrActivity;
import com.mybitcoin.wallet.ui.WalletActivity;
import com.mybitcoin.wallet.util.CrashReporter;
import com.mybitcoin.wallet.util.GenericUtils;
import com.mybitcoin.wallet.util.ThrottlingWalletChangeListener;
...
//							notifyCoinsReceived(from, amount);
						...
						// AUTHOR: F, DATE: 2014.6.4
						if (isReceived && !isReplayedTx) {
							String transHash = tx.getHashAsString();
							String payerAddrStr = from.toString();
							
							String formattedAmountStr = GenericUtils.formatValue(amount, 8, 0);	// 单位：比特币，精度：小数点后8位	
//							Log.d("BLOCKCHAIN", "Coin Received, TransHash: " + transHash + ", BTC Amount: " + formattedAmountStr + ", Payer Addr: " + payerAddrStr);
							
							String tradeID = CashTradeInfoLog.queryMatchedTradeIdByBTC(getContentResolver(), formattedAmountStr);
							if(tradeID == null) {
//								Log.d("BLOCKCHAIN", "The Received Coin doesn't match any INITED state Trades by Btc Amount: " + formattedAmountStr + ", Payer Addr: " + payerAddrStr);
								return;
							}
							
							CashTradeInfoLog tradeInfo = new CashTradeInfoLog(getContentResolver(), tradeID);
							
							// 设置当前交易为PAYING状态，并填充支付者地址信息
							tradeInfo.setPayingState(transHash, payerAddrStr, null);
//							Log.d("BLOCKCHAIN", "Set Paying State for TradeID: " + tradeID + ", TransHash: " + transHash + ", BTC Amount: " + formattedAmountStr + ", Payer Addr: " + payerAddrStr);
						}
					}
				});
			}
